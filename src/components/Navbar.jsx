    ...
    <header className="sticky top-0 z-40 mb-4 border-b border-zinc-800/80 bg-cinema-bg/90 backdrop-blur sm:mb-6">
      <div className="flex h-16 items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button className="relative rounded-lg p-2 text-zinc-400 transition-colors hover:bg-white/5 hover:text-white" aria-label="Thông báo">
            <Bell className="h-5 w-5" />
            <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-cinema-primary" />
          </button>

          <Link to="/movies" className="cinema-btn-primary hidden px-4 py-2 md:inline-flex">
            <Ticket className="h-4 w-4" />
            Mua vé
          </Link>

          <button className="hidden items-center gap-1 rounded-lg p-1.5 text-zinc-400 transition-colors hover:bg-white/5 hover:text-white sm:flex" aria-label="Tài khoản">
      ...
      {menuOpen ? (
        <div className="border-t border-zinc-800/80 py-3 md:hidden">
          <div className="space-y-1">
            ...
            <button onClick={() => navigate('/movies')} className="cinema-btn-primary mt-3 w-full justify-center px-4 py-2.5">
              <Ticket className="h-4 w-4" />
              Mua vé
            </button>
          </div>

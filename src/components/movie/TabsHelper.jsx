...
import { Star, Clock, User, Heart, Info, ChevronRight, Pencil, X, CheckCircle2, AlertCircle, ThumbsUp } from "lucide-react";
...
export const ShowtimesTab = ({ showtimes }) => {
  ...
  const currentShowtimes = dates[selectedDate] ? groupedDates[dates[selectedDate]] : {};
    ...
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-10">
      <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4" style={{ scrollbarWidth: "none" }}>
          {dates.length === 0 && <p className="text-zinc-500 text-sm">Chưa có lịch chiếu cho phim này.</p>}
          {dates.map((d, index) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[70px] h-[90px] rounded-2xl border transition-all shrink-0 ${active ? 'border-red-600 text-white bg-[#18181b]' : 'border-zinc-800 text-zinc-500 bg-[#111113] hover:bg-zinc-900'}`}
              >
                <div className={`text-xs font-bold mb-1 ${active ? 'text-white' : 'text-zinc-500'}`}>{dayOfWeek}</div>
                <div className={`text-2xl font-black mb-1 ${active ? 'text-red-500' : 'text-zinc-300'}`}>{date}</div>
                <div className={`text-xs font-bold ${active ? 'text-white' : 'text-zinc-500'}`}>T{month}</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="space-y-4">
        {Object.keys(currentShowtimes).map(cinemaName => {
          ...
          const firstWithAddress = cinemaShows.find(s => s.cinemaAddress || s.address) || cinemaShows[0] || {};
          const displayAddress = firstWithAddress.cinemaAddress || firstWithAddress.address || "Đang cập nhật địa chỉ";

          return (
            <div key={cinemaName} className="bg-[#111113] border border-zinc-800/80 rounded-[20px] p-5 shadow-lg">
              <div
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-red-600 flex items-center justify-center shrink-0 shadow-md">
                    <span className="text-white font-black text-sm">{cinemaName.split(' ')[0]}</span>
                  </div>
                  <div>
                    <h4 className="text-white font-bold text-[15px] group-hover:text-red-500 transition-colors uppercase">{cinemaName}</h4>
                    <div className="flex items-center gap-1 text-zinc-500 text-xs mt-1">
                      <Info className="w-3 h-3" />
                      <span className="truncate max-w-[200px] sm:max-w-md">{displayAddress}</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-zinc-400 text-sm font-semibold whitespace-nowrap">
                  <span className="hidden sm:inline">{cinemaShows.length} suất</span>
                  <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${isExpanded ? 'rotate-90' : 'rotate-0'}`} />
                </div>
              </div>
...
export const ReviewsTab = ({ movieId, reviews, onReviewSubmit }) => {
  ...
  const handleLike = async (reviewId) => {
    if (!user) return showNotification("error", "Vui lòng đăng nhập để thích bài viết");
    try {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!user) return showNotification("error", "Vui lòng đăng nhập để đánh giá");
    if (!comment.trim()) return showNotification("error", "Vui lòng nhập bình luận");
    
    if (existingReview && !isEditing) {
      showNotification("warning", "Bạn đã đánh giá phim này rồi. Hãy sử dụng nút chỉnh sửa bên dưới.");
      return;
    }
    ...
    try {
      const res = await fetch("http://localhost:5000/api/reviews", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: user.user_id, movie_id: movieId, rating, comment })
      });
      if (res.ok) {
        showNotification("success", isEditing ? "Cập nhật đánh giá thành công!" : "Đã gửi đánh giá của bạn!");
        setComment("");
        setRating(5);
        setIsEditing(false);
        if (onReviewSubmit) onReviewSubmit();
        ...
        showNotification("error", `Có lỗi xảy ra: ${errData.message || "Vui lòng thử lại sau"}`);
      }
    } catch {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-10">
      ...
      <div className="fixed top-24 right-4 z-[100] space-y-3 pointer-events-none">
        {notif && (
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border border-white/10 shadow-2xl backdrop-blur-xl animate-in slide-in-from-right-4 duration-300 pointer-events-auto ${
            notif.type === 'success' ? 'bg-green-500/90 text-white' : 
            notif.type === 'warning' ? 'bg-amber-500/90 text-white' : 'bg-red-500/90 text-white'
          }`}>
            ...
            <button onClick={() => setNotif(null)} className="ml-2 hover:opacity-70">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6 flex flex-col sm:flex-row gap-6 items-center sm:items-start shadow-xl">
        <div className="flex flex-col items-center justify-center shrink-0">
          ...
          <div className="flex mt-2 mb-1">
            {[1, 2, 3, 4, 5].map(i => (
              <Star key={i} className={`w-4 h-4 ${i <= Math.round(averageRating) ? 'fill-yellow-400 text-yellow-400' : 'text-zinc-600'}`} />
            ))}
          </div>
          ...
          {[1, 2, 3, 4, 5].map(star => {
            ...
            return (
              <div key={star} className="flex items-center gap-3 text-sm">
                ...
                <div className="text-zinc-500 w-4 text-xs text-right">{ratingCounts[star]}</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="flex justify-between items-center text-sm border-b border-zinc-800 pb-4 mt-8">
        <span className="text-zinc-100 font-bold text-base">{reviews.length} đánh giá</span>
        <div className="flex p-1 bg-zinc-900/90 border border-zinc-800/50 rounded-full shadow-inner">
          <button 
            onClick={() => setFilter('helpful')}
            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${filter === 'helpful' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Hữu ích nhất
          </button>
          <button 
            onClick={() => setFilter('rating')}
            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${filter === 'rating' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Điểm cao
          </button>
          <button 
            onClick={() => setFilter('newest')}
            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${filter === 'newest' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Mới nhất
          </button>
        </div>
      ...
      {/* WRITE REVIEW */}
      <div ref={formRef} className={`bg-zinc-900/60 border rounded-2xl p-6 mb-4 transition-colors ${isEditing ? 'border-amber-500/50' : 'border-zinc-800'}`}>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-white font-bold">{isEditing ? 'Chỉnh sửa đánh giá của bạn' : 'Viết đánh giá của bạn'}</h3>
          {isEditing && (
            <button onClick={cancelEdit} className="text-zinc-400 hover:text-white flex items-center gap-1 text-xs font-bold uppercase transition-colors">
        ...
        {user ? (
          <div className="flex items-center gap-3 mb-6">
            {user.photoURL ? (
              <img src={user.photoURL} alt={user.displayName} className="w-10 h-10 rounded-full object-cover border border-zinc-700" />
            ) : (
            ...
            <div>
              <p className="text-white font-bold text-sm">{user.displayName || "Thành viên"}</p>
              <p className="text-zinc-500 text-xs">{isEditing ? 'Đang cập nhật đánh giá...' : 'Đang viết đánh giá...'}</p>
            </div>
          </div>
        ) : null}
        {user ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <p className="text-zinc-400 text-xs uppercase tracking-wide font-semibold mb-2">Điểm đánh giá *</p>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(i => (
                  <Star
                    key={i}
                    className={`w-8 h-8 cursor-pointer transition-colors ${i <= rating ? 'fill-yellow-400 text-yellow-400' : 'text-zinc-600 hover:text-zinc-400'}`}
                    onClick={() => setRating(i)}
                  />
                ))}
              </div>
            ...
            <div>
              <p className="text-zinc-400 text-xs uppercase tracking-wide font-semibold mb-2">Nhận xét *</p>
              <textarea
            ...
            <div className="flex justify-end gap-3">
              {isEditing && (
                <button type="button" onClick={cancelEdit} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold px-6 py-2 rounded-xl transition-colors">
                  Hủy
                </button>
              )}
              <button disabled={submitting} type="submit" className={`${isEditing ? 'bg-amber-600 hover:bg-amber-700' : 'bg-red-600 hover:bg-red-700'} text-white font-bold px-6 py-2 rounded-xl transition-colors disabled:opacity-50 shadow-lg`}>
                {submitting ? 'Đang gửi...' : isEditing ? 'Cập nhật ngay' : 'Gửi bài đánh giá'}
              </button>
            </div>
          </form>
        ) : (
          <div className="text-center py-6 border border-zinc-800 rounded-xl bg-zinc-900/30">
            <p className="text-zinc-400 mb-3 text-sm">Vui lòng đăng nhập để có thể để lại bài đánh giá cho bộ phim này.</p>
            <button onClick={() => navigate('/auth')} className="bg-white text-black font-bold uppercase tracking-wide text-xs px-6 py-2.5 rounded-full shadow-lg">Đăng nhập ngay</button>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {sortedReviews.map((r) => {
          ...
          return (
            <div
              key={r.review_id}
              className={`bg-[#111116] border rounded-2xl p-6 transition-all duration-300 ${isCurrentUser ? 'border-red-600/40 ring-1 ring-red-600/10' : 'border-zinc-800/80 hover:border-zinc-700'}`}
            >
                ...
                <div className="flex items-center gap-3">
                  {finalAvatar ? (
                    ...
                    <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-red-600 to-red-400 flex items-center justify-center text-white font-bold text-lg shrink-0 border-2 border-zinc-800">
                      {finalName ? finalName.substring(0, 1).toUpperCase() : "U"}
                    </div>
                  )}
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <Star
                            key={i}
                            className={`w-3.5 h-3.5 ${i <= r.rating ? "fill-yellow-400 text-yellow-400" : "text-zinc-700"
                              }`}
                          />
                        ))}
                      </div>
              ...
              <div className="flex items-center gap-4 pt-3 border-t border-zinc-800/50">
                ...
                >
                  ...
                  <span className="text-[11px] font-bold">{likeCount} lượt thích</span>
                </button>
              </div>
            </div>
          );

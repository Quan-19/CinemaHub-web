...
import {
  ChevronLeft, Check, CreditCard, Smartphone, Wallet, Plus, Minus,
  Tag, Ticket, Shield, Star, MapPin, Calendar, Clock, Film,
  Download, Share2, QrCode, Armchair, CheckCircle, Sparkles,
} from "lucide-react";
...
export const BookingConfirmationPage = () => {
  ...
  const isFromTicketPage = Boolean(location.state?.fromTicket && ticketFromState);

  const movie = selectedMovie || (ticketFromState
    ? {
    ...
    : null);
  const cinema = selectedCinema || (ticketFromState
    ? {
    ...
    : null);
  const showtime = selectedShowtime || (ticketFromState
    ? {
    ...
    : null);
  const seats = selectedSeats?.length > 0 ? selectedSeats : (ticketFromState?.seats || []);
  ...
  const ticketTotal = isFromTicketPage
    ...
    : seats.reduce((sum, seat) => {
        ...
        const mult = seat.type === "vip" ? 1.3 : seat.type === "couple" ? 1.5 : 1;
      ...
      <div className="min-h-screen pt-16" style={{ background: "#0a0a0f" }}>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
          ...
          <motion.div
            ...
            style={{ background: "linear-gradient(135deg, #0d2a0d, #0a1a0a)", border: "1px solid rgba(34,197,94,0.3)" }}
          >
            {/* Glow */}
            <div className="absolute inset-0 pointer-events-none"
              style={{ background: "radial-gradient(ellipse at 50% -20%, rgba(34,197,94,0.15) 0%, transparent 70%)" }} />
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <h2 className="text-white" style={{ fontSize: "1.4rem", fontWeight: 800 }}>Thanh toán thành công!</h2>
                <Sparkles size={18} style={{ color: "#f59e0b" }} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            >
              ...
              <div className="rounded-2xl p-5" style={{ background: "#12121f", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-white" style={{ fontWeight: 700, fontSize: 15 }}>Thông tin vé</h3>
                </div>

                <div className="flex gap-3 mb-4">
                  <img src={movie.poster} alt={movie.title}
                    className="w-16 h-22 object-cover rounded-xl flex-shrink-0"
                    style={{ width: 64, height: 88, border: "1px solid rgba(255,255,255,0.1)" }} />
                  <div className="flex-1">
                    <div className="text-white" style={{ fontWeight: 700, fontSize: 14 }}>{movie.title}</div>
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      <span className="px-1.5 py-0.5 rounded text-xs font-bold"
                        style={{ background: `${RATING_COLOR[movie.rating] || "#e50914"}25`, color: RATING_COLOR[movie.rating] || "#e50914" }}>
                        {movie.rating}
                      </span>
                      <span className="px-1.5 py-0.5 rounded text-xs font-bold"
                ...
                <div className="space-y-2.5">
                  {[
                    { icon: <MapPin size={12} />, label: "Rạp", value: cinema.name, color: "#e50914" },
                    { icon: <Calendar size={12} />, label: "Ngày", value: date || "—", color: "#06b6d4" },
                    { icon: <Clock size={12} />, label: "Giờ chiếu", value: `${showtime.time}`, color: "#22c55e" },
                    { icon: <Film size={12} />, label: "Phòng", value: showtime.roomId, color: "#8b5cf6" },
                  ].map(row => (
                    <div key={row.label} className="flex items-center gap-2.5">
                      <span style={{ color: row.color, opacity: 0.8 }}>{row.icon}</span>
                      <span className="text-zinc-500" style={{ fontSize: 12, minWidth: 64 }}>{row.label}</span>
                      <span className="text-white" style={{ fontSize: 13, fontWeight: 600 }}>{row.value}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-3 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Ghế đã đặt</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {seats.map(seat => (
                      <span key={seat.id}
                        ...
                        style={{
                          background: seat.type === "vip" ? "rgba(245,158,11,0.15)" : seat.type === "couple" ? "rgba(229,9,20,0.15)" : "rgba(255,255,255,0.08)",
              ...
              {/* Payment summary */}
              <div className="rounded-2xl p-5" style={{ background: "#12121f", border: "1px solid rgba(255,255,255,0.07)" }}>
                <h3 className="text-white mb-3" style={{ fontWeight: 700, fontSize: 14 }}>Chi tiết thanh toán</h3>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                    <span className="text-white" style={{ fontWeight: 700 }}>Tổng thanh toán</span>
                    <span style={{ color: "#f59e0b", fontWeight: 800, fontSize: 16 }}>{grandTotal.toLocaleString()}₫</span>
                  </div>
            ...
            >
              ...
              <div className="rounded-2xl p-5 text-center" style={{ background: "#12121f", border: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div className="text-white text-3xl tracking-[0.2em] mb-1" style={{ fontFamily: "monospace", fontWeight: 900 }}>
                  {bookingCode}
                </div>
                <div className="flex items-center justify-center gap-1.5">
                  <CheckCircle size={13} style={{ color: "#22c55e" }} />
                  <span className="text-green-400 text-xs">Đã xác nhận · {new Date().toLocaleDateString("vi-VN")}</span>
                </div>
              </div>

              {/* QR code card */}
              <div className="rounded-2xl overflow-hidden" style={{ background: "#12121f", border: "1px solid rgba(229,9,20,0.25)" }}>
                {/* Header */}
                <div className="px-5 py-3 flex items-center gap-2"
                  style={{ background: "linear-gradient(90deg, rgba(229,9,20,0.15), rgba(229,9,20,0.05))", borderBottom: "1px solid rgba(229,9,20,0.15)" }}>
                  <QrCode size={16} style={{ color: "#e50914" }} />
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>Vé điện tử (E-Ticket)</span>
                </div>

                <div className="p-5 flex flex-col items-center">
                  {/* QR */}
                  <div className="p-3 rounded-2xl mb-3 relative" style={{ background: "#fff" }}>
                    <QRCodeSVG
                      value={ticketUrl}
                      size={160}
                      bgColor="#ffffff"
                  ...
                  <p className="text-center text-zinc-400 text-xs mb-4" style={{ lineHeight: 1.6 }}>
                  ...
                  {/* View ticket button */}
                  <button
                    onClick={() => navigate(`/ticket/${bookingCode}`, {
                      state: {
                        fromSuccess: true,
                        ticketData: {
                          bookingCode,
              ...
              <div className="rounded-xl p-4 flex gap-3"
                ...
                <div>
                  <p className="text-amber-400 text-xs" style={{ fontWeight: 600, marginBottom: 4 }}>Lưu ý quan trọng</p>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 justify-center mb-4">
                  ...
                  <span style={{ fontSize: 16, fontWeight: 800, color: "#e50914" }}>CINE</span>
                  <span style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>STAR</span>
                </div>
                <div className="p-4 rounded-2xl mb-4 inline-block" style={{ background: "#fff" }}>
                  <QRCodeSVG
                    value={ticketUrl}
                    size={240}
                    bgColor="#ffffff"
                ...
                <p className="text-zinc-500 text-xs mb-4">{showtime.time} · {cinema.name}</p>
                <div className="text-white text-xl tracking-[0.15em] mb-4" style={{ fontFamily: "monospace", fontWeight: 900 }}>
                  {bookingCode}
                </div>
      ...
      <div className="border-b border-zinc-800" style={{ background: "#12121f" }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-white" style={{ fontWeight: 700 }}>Xác nhận đặt vé</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            {/* Booking info */}
            <div className="rounded-2xl border border-zinc-800 p-5" style={{ background: "#12121f" }}>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-2">
                  <div>
                    <p className="text-white" style={{ fontWeight: 700 }}>{movie.title}</p>
                    <p className="text-zinc-500 text-xs">{movie.originalTitle}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      <p className="text-zinc-500 text-xs">Rạp chiếu</p>
                      <p className="text-zinc-200 text-xs" style={{ fontWeight: 600 }}>{cinema.name}</p>
                    </div>
                    <div>
                      <p className="text-zinc-500 text-xs">Suất chiếu</p>
                      <p className="text-zinc-200 text-xs" style={{ fontWeight: 600 }}>{showtime.time} · {showtime.type}</p>
                    </div>
                    <div>
                      <p className="text-zinc-500 text-xs">Phòng</p>
                      <p className="text-zinc-200 text-xs" style={{ fontWeight: 600 }}>{showtime.roomId}</p>
                    </div>
                    <div>
                      <p className="text-zinc-500 text-xs">Ghế</p>
                      <p className="text-zinc-200 text-xs" style={{ fontWeight: 600 }}>{seats.map((s) => s.id).join(", ")}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Combos */}
            <div className="rounded-2xl border border-zinc-800 p-5" style={{ background: "#12121f" }}>
              <h3 className="text-white mb-4" style={{ fontWeight: 600 }}>🍿 Thêm combo bắp nước (tuỳ chọn)</h3>
              <div className="space-y-3">
                {COMBOS.map((combo) => {
                  const count = comboCounts[combo.id] || 0;
                  return (
                    <div key={combo.id} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900 border border-zinc-800">
                      <div>
                        <p className="text-zinc-200 text-sm">{combo.name}</p>
                        <p className="text-red-400 text-xs mt-0.5" style={{ fontWeight: 600 }}>{combo.price.toLocaleString()}₫</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setComboCounts((c) => ({ ...c, [combo.id]: Math.max(0, (c[combo.id] || 0) - 1) }))}
                          disabled={count === 0}
                        ...
                        <span className="w-5 text-center text-white text-sm" style={{ fontWeight: 700 }}>{count}</span>
                        <button
                          onClick={() => setComboCounts((c) => ({ ...c, [combo.id]: (c[combo.id] || 0) + 1 }))}
                ...
                {PAYMENT_METHODS.map((method) => (
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${
                      paymentMethod === method.id ? "border-red-500 bg-red-500/10" : "border-zinc-700 bg-zinc-900 hover:border-zinc-600"
                    }`}
                  >
                    <span className="text-xl">{method.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs truncate ${paymentMethod === method.id ? "text-white" : "text-zinc-300"}`} style={{ fontWeight: 600 }}>
                        {method.label}
                      </p>
                      <p className="text-zinc-500 text-xs">{method.desc}</p>
                    </div>
                    {paymentMethod === method.id && (
                      <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "#e50914" }}>
          ...
          <div className="lg:w-72">
            <div className="rounded-2xl border border-zinc-800 p-5 sticky top-20" style={{ background: "#12121f" }}>
              <h3 className="text-white mb-4" style={{ fontWeight: 700 }}>Tóm tắt đơn hàng</h3>
              ...
              <div className="flex justify-between items-center py-3 border-t border-zinc-700 mb-4">
                <span className="text-white" style={{ fontWeight: 700 }}>Tổng thanh toán</span>
                <span className="text-lg" style={{ color: "#e50914", fontWeight: 800 }}>{grandTotal.toLocaleString()}₫</span>
              </div>

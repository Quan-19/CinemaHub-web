...
const VN_AREA_DEFINITIONS = [
  {
    ...
    id: "ha-noi",
    aliases: ["ha noi", "hanoi", "hn", "tp ha noi"],
  },
  {
    label: "Thành phố Huế",
    id: "hue",
    aliases: ["hue", "thanh pho hue", "tp hue", "thua thien hue"],
  },
  {
    label: "Thành phố Hồ Chí Minh",
    id: "ho-chi-minh",
    aliases: ["ho chi minh", "tp ho chi minh", "tphcm", "hcm", "sai gon"],
  },
  {
  ...
  },
  {
    ...
    id: "can-tho",
    aliases: ["can tho", "cantho", "tp can tho"],
  },
...
const VIETNAM_CITY_ENTRIES = (() => {
  ...
  VN_AREA_DEFINITIONS.forEach((canonical) => {
    ...
    const shortLabel = normalizeCityText(canonical.label)
...
const inferCinemaCity = (cinema) => {
  ...
  const segments = String(cinema?.address ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
    .slice(-3)
...
function CinemaPage() {
  ...
  useEffect(() => {
    const fetchShowtimes = async () => {
      ...
      try {
        let res = await fetch(
          `${API_BASE_URL}/api/showtimes/cinema/${selectedCinemaId}`
        );
        ...
        const todayShowtimes = rawData.filter((st) => {
          ...
          if (stCinemaId && String(stCinemaId) !== String(selectedCinemaId))
            return false;
  ...
  const nowShowingMovies = useMemo(() => {
    ...
    const movieIdsAtCinema = new Set(
      cinemaShowtimes.map((st) => String(st.movie_id || st.movieId))
    );
    return allNowShowing.filter((m) =>
      movieIdsAtCinema.has(String(m.id || m.movie_id))
    );
  ...
  return (
    <main className="min-h-screen bg-cinema-bg pb-24 overflow-x-hidden">
      {/* HERO SECTION */}
      <header className="relative pt-[20px] pb-[20px] overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      ...
                      <p className="text-zinc-500 text-sm line-clamp-1">
                        Khu vực{" "}
                        {region.label.replace(/^(Tỉnh|Thành phố)\s+/i, "")}
                      </p>
                    </div>
                  ...
                  {!selectedCinema ? (
                    <div className="h-[50vh] flex flex-col items-center justify-center glass-card rounded-[40px] border-dashed border-white/10 text-center p-12">
                      ...
                      <p className="text-zinc-500 max-w-sm">
                        Chọn rạp bên trái để xem lịch chiếu và các thông tin chi
                        tiết khác.
                      </p>
                    </div>
                  ) : (
                    ...
                    >
                      ...
                      <div>
                        ...
                        {nowShowingMovies.length > 0 ? (
                          ...
                          <div className="text-center py-12 glass-card rounded-3xl mt-4">
                            <Ticket className="w-12 h-12 text-zinc-500 mx-auto mb-4 opacity-50" />
                            <h4 className="text-white text-lg font-bold">
                              Chưa có suất chiếu
                            </h4>
                            <p className="text-zinc-400 mt-2 text-sm">
                              Hiện tại rạp này chưa có suất chiếu nào lên lịch
                              hôm nay.
                            </p>
                          </div>
                        )}

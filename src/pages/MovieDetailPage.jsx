...
import { Star, Clock, Calendar, Globe, User, Play, Ticket, ChevronLeft, Share2, Heart } from "lucide-react";
...
export const MovieDetailPage = () => {
  ...
  const related = MOVIES.filter((m) => m.id !== id && m.genre.some((g) => movie?.genre.includes(g))).slice(0, 4);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#0a0a0f" }}>
        <div className="text-center">
          ...
          <button onClick={() => navigate("/movies")} className="text-red-500 hover:underline">Quay lại</button>
        </div>
      </div>
    );
  }
  ...
  const movieDetails = [
    ...
    { icon: Calendar, label: "Khởi chiếu", value: movie.releaseDate || "Đang cập nhật" },
    ...
    <div className="min-h-screen pt-16" style={{ background: "#0a0a0f" }}>
      ...
      <div className="relative h-[50vh] min-h-[400px] overflow-hidden">
        ...
        <div
          ...
          style={{ background: "linear-gradient(to top, #0a0a0f 10%, rgba(10,10,15,0.4) 60%, rgba(10,10,15,0.2) 100%)" }}
        />
        <div
          className="absolute inset-0"
          style={{ background: "linear-gradient(to right, rgba(10,10,15,0.6) 0%, transparent 60%)" }}
        />
        ...
        <div className="absolute top-4 right-4 sm:right-6 flex gap-2">
          <button
            ...
            className={`w-9 h-9 rounded-full bg-black/40 border border-white/10 flex items-center justify-center transition-colors ${liked ? "text-red-500" : "text-white hover:text-red-400"}`}
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10 pb-16">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span
                ...
                style={{ background: movie.status === "now-showing" ? "#e50914" : "#f59e0b", fontWeight: 700 }}
              >
                {movie.status === "now-showing" ? "Đang chiếu" : "Sắp chiếu"}
              </span>
              <span
                className="px-2.5 py-1 rounded-lg text-xs text-white"
                style={{ background: ratingColors[movie.rating], fontWeight: 700 }}
              >
                {movie.rating}
              </span>
              {movie.genre.map((g) => (
                <span key={g} className="px-2.5 py-1 rounded-lg text-xs bg-zinc-800 text-zinc-300 border border-zinc-700">
                  {g}
                </span>
              ))}
            </div>

            <h1 className="text-white mb-1" style={{ fontSize: "2rem", fontWeight: 800, lineHeight: 1.2 }}>
              {movie.title}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-zinc-500 text-xs">({voteCount} lượt)</span>
              </div>
            </div>

            {/* Meta info */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
              {movieDetails.map(({ icon: Icon, label, value }) => (
            ...
            {/* Description */}
            <div className="mb-5">
              <h3 className="text-white mb-2" style={{ fontWeight: 600 }}>Nội dung phim</h3>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              <div>
                <p className="text-zinc-500 text-xs mb-1.5 uppercase tracking-wider">Đạo diễn</p>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center border border-zinc-700">
                    <User className="w-4 h-4 text-zinc-400" />
                  </div>
                  <span className="text-white text-sm" style={{ fontWeight: 600 }}>{director}</span>
                </div>
              </div>
              <div>
                <p className="text-zinc-500 text-xs mb-1.5 uppercase tracking-wider">Diễn viên</p>
                <div className="flex flex-wrap gap-1">
                  {cast.length > 0 ? cast.map((actor) => (
                    <span key={actor} className="bg-zinc-800 text-zinc-300 text-xs px-2 py-1 rounded-lg border border-zinc-700">
                      {actor}
                    </span>
        ...
        {related.length > 0 && (
          <div className="mt-12">
            <h2 className="text-white mb-6" style={{ fontSize: "1.25rem", fontWeight: 700 }}>
              Phim liên quan
            </h2>
          ...
          >
            <div className="relative bg-zinc-900" style={{ paddingTop: "56.25%" }}>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <p className="text-zinc-600 text-sm mt-1">(Demo - Trailer không khả dụng trong bản demo)</p>
              </div>
            </div>
            <button
              onClick={() => setShowTrailer(false)}

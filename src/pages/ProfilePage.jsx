...
function ProfilePage() {
    ...
    const fetchBookings = async () => {
      ...
      try {
        ...
        for (const booking of data) {
          ...
          let posterUrl = booking.poster_url
            ? getFullPosterUrl(booking.poster_url)
            : await fetchMoviePoster(booking.movie_name);
          formatted.push({
            booking_id: booking.booking_id,
            booking_date: bookingDate,
            booking_date_raw: booking.booking_date,
            total_amount: parseFloat(booking.total_amount || 0),
            original_ticket_amount: parseFloat(
              booking.original_ticket_amount || 0
            ),
  ...
  const filteredBookings = bookings.filter((b) => {
    const matchesSearch = (b.movie_name || "")
      .toLowerCase()
  ...
  const getPosterUrl = (booking) =>
    posterErrors[booking.booking_id]
      ? FALLBACK_POSTER
      : booking.poster_url || FALLBACK_POSTER;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0a0a0f]">
        <motion.div
          ...
          className="h-10 w-10 border-4 border-cinema-primary border-t-transparent rounded-full"
        />
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            >
              Hồ sơ{" "}
              <span className="text-cinema-primary text-gradient">Cá nhân</span>
            </motion.h1>
            <p className="mt-1 text-zinc-500 text-sm">
              Quản lý thông tin và theo dõi lịch sử đặt vé của bạn.
            </p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => navigate("/")}
              className="px-4 py-2 rounded-xl bg-zinc-800/50 border border-zinc-700 hover:bg-zinc-800 transition-all flex items-center gap-2 text-xs font-semibold"
            >
              <ArrowLeft className="h-4 w-4" /> Quay lại
            </button>
            <button
              onClick={handleLogout}
              className="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 text-red-500 transition-all flex items-center gap-2 text-xs font-semibold"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <p className="text-zinc-500 text-xs mb-6 truncate max-w-full">
                {user.email}
              </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 rounded-3xl border border-white/5 bg-zinc-900/30 space-y-5">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-semibold text-white px-1 truncate">
                          {user.email}
                        </p>
                      </div>
                      <div className="space-y-1">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-11 pr-4 py-3 bg-zinc-900 border border-zinc-800 rounded-2xl text-sm text-white focus:border-cinema-primary outline-none transition-all"
                          />
                        </div>
                        {phoneError && (
                      ...
                      <button
                        ...
                        className={`w-full py-3.5 rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                          saved
                            ...
                            : "bg-cinema-primary hover:bg-cinema-primary/90"
                        } text-white`}
                      >
                        {saving ? (
                ...
                >
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-zinc-500 uppercase px-2 py-0.5 bg-zinc-900 rounded-lg border border-zinc-800 ml-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                      ...
                      <div className="flex items-center gap-1 bg-zinc-900/50 p-1 rounded-xl border border-white/5">
                        {["all", "paid", "pending", "cancelled"].map(
                          (status) => (
                            <button
                              key={status}
                              onClick={() => setFilterStatus(status)}
                              className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all uppercase tracking-tighter ${
                                filterStatus === status
                  ...
                  {loadingHistory ? (
                    <div className="py-20 text-center text-zinc-500 text-sm font-medium italic">
                      Đang tải dữ liệu...
                    </div>
                    ...
                    <div className="py-20 text-center bg-zinc-900/20 border border-dashed border-zinc-800 rounded-3xl">
                      ...
                      {(searchTerm || filterStatus !== "all") && (
                        <button
                          ...
                          className="mt-4 px-6 py-2 bg-zinc-800 text-white rounded-xl text-xs font-bold hover:bg-zinc-700 transition-all"
                        >
                          Đặt lại bộ lọc
                        </button>
                      )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredBookings.map((booking) => (
                          <div
                            ...
                            className="group p-3 border border-white/5 bg-zinc-900/40 rounded-[1.5rem] hover:border-cinema-primary/30 transition-all cursor-pointer flex gap-4 overflow-hidden"
                          >
                            <img
                              src={getPosterUrl(booking)}
                              alt={booking.movie_name}
                              className="w-20 h-28 rounded-xl object-cover shadow-lg group-hover:scale-105 transition-transform duration-500"
                              onError={() =>
                                handlePosterError(booking.booking_id)
                              }
                            />
                            <div className="flex-1 flex flex-col justify-between py-1">
                              ...
                              <div className="flex justify-between items-center pt-2 border-t border-zinc-800/50">
                                <span className="text-cinema-primary font-bold text-sm leading-none">
                                  {formatCurrency(booking.total_amount)}
                                </span>
                                <div
                                  className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-tighter flex items-center gap-1 ${
        ...
        {showModal && selectedBooking && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-[#12121e] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col"
            ...
            >
              <div className="p-5 border-b border-white/5 flex justify-between items-center">
                ...
                <button
                  onClick={() => setShowModal(false)}
                  className="p-1 hover:bg-white/5 rounded-full text-zinc-500"
                >
                  <X />
                </button>
              </div>
                ...
                <div className="pt-6 border-t border-zinc-800 flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <div className="space-y-2 bg-white/[0.03] p-3 rounded-2xl border border-white/10 max-w-[220px]">
                      ...
                      <div className="flex justify-between items-center text-[11px]">
                        ...
                        <span className="text-white font-bold">
                          {formatCurrency(
                            selectedBooking.original_ticket_amount > 0
                              ? selectedBooking.original_ticket_amount
                              : selectedBooking.ticket_amount
                          )}
                        </span>
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-3 bg-white/5 rounded-2xl border border-white/10 shrink-0">
                    {selectedBooking.payment_status === "paid" ? (
                        ...
                        <div className="bg-white p-1.5 rounded-lg mb-2">
                          <QRCodeSVG
                            value={
                              ...
                              selectedBooking.ticket_code ||
                              String(selectedBooking.booking_id)
                            }
                            size={85}
                            level="H"
                          />
                      ...
                      <>
                        <div className="w-[85px] h-[85px] rounded-lg mb-2 bg-zinc-900/60 border border-white/10 flex items-center justify-center">
                  ...
                  <div className="text-right flex flex-col justify-between h-full py-1 leading-none">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-cinema-primary leading-none tracking-tight">
              ...
              <div className="p-6 bg-zinc-900/50">
                <button
                  onClick={() => setShowModal(false)}
                  className="w-full py-3 bg-zinc-100 hover:bg-white text-black font-bold rounded-xl transition-all"
                >
                  Đóng
                </button>
              </div>

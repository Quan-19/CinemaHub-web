        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROMOTIONS.map((promo) => (
            ...
            >
              ...
              <div className="p-5">
                <h3 className="text-white mb-2" style={{ fontWeight: 700 }}>{promo.title}</h3>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 bg-zinc-900 border border-dashed border-zinc-600 rounded-xl px-4 py-2.5 flex items-center gap-2">
                    ...
                    <span className="text-white text-sm tracking-wider" style={{ fontWeight: 700 }}>
                      {promo.code}
                    </span>
                  </div>
                  <button
                    onClick={() => handleCopy(promo.code)}
                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${copied === promo.code ? "bg-green-600" : "bg-zinc-700 hover:bg-zinc-600"}`}
                  >
                    {copied === promo.code ? <Check className="w-4 h-4 text-white" /> : <Copy className="w-4 h-4 text-white" />}
                  </button>
                </div>
              </div>
            </div>

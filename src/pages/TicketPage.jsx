...
const Barcode = ({ value }) => {
  ...
  while (padded.length < 60)
    padded.push({
      w: 1,
      h: 28 + (padded.length % 10),
      key: padded.length + 1000,
    });
...
const normalizeSeatType = (value) => {
  ...
  if (raw.includes("couple") || raw.includes("double") || raw.includes("doi"))
    return "couple";
  if (
...
export const TicketPage = () => {
    ...
    const load = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/tickets/${encodeURIComponent(code)}`
        );
        const data = res.data;
        ...
        const mapped = {
          bookingCode:
            data?.ticket_code || data?.booking_code || data?.code || code,
          bookingStatus:
            data?.booking_status || data?.bookingStatus || data?.status,
          customerName: data?.customer_name || data?.customerName || "",
          cinemaName:
            data?.cinema?.name || data?.cinema_name || data?.cinemaName || "",
          cinemaAddress:
            data?.cinema?.address ||
            data?.cinema_address ||
            data?.cinemaAddress ||
          ...
          roomName:
            ...
            "",
          showtimeType:
            data?.showtime?.format ||
            data?.showtime?.show_format ||
            data?.showtime?.room?.type ||
            "",
          showtimeLanguage: data?.showtime?.language || "",
          movieTitle: data?.movie?.title || data?.movie_title || "",
          movieOriginalTitle:
            data?.movie?.original_title || data?.original_title || "",
          ...
          movieRating: normalizeAgeRating(
            data?.movie?.age_rating || data?.age_rating
          ),
          grandTotal: Math.round(Number(data?.total_price || 0)),
          issuedAt: createdAt ? createdAt.toLocaleString("vi-VN") : "",
          seats: (data?.seats || []).map((s) => ({
            id: normalizeSeatLabel(
              s.label || s.seat_number || s.seat_id || s.id
            ),
    ...
    return (
      ...
      >
        ...
        <p className="text-zinc-400 text-sm mb-6 text-center">
          Mã vé không hợp lệ hoặc đã hết hạn
        </p>
        <button
          onClick={() => navigate("/")}
  ...
  const bookingStatus = String(ticket.bookingStatus || "").toLowerCase();
  const isPaid = bookingStatus === "paid" || bookingStatus === "confirmed";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ filter: "drop-shadow(0 20px 60px rgba(229,9,20,0.25))" }}
      >
        {/* Top section */}
        <div
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                <span
                  ...
                  style={{
                    background: `${
                      RATING_COLOR[ticket.movieRating] || "#e50914"
                    }20`,
                    color: RATING_COLOR[ticket.movieRating] || "#e50914",
                  }}
                >
                  {ticket.movieRating}
                </span>
                ...
                {ticket.showtimeLanguage && (
                  ...
                  >
                    {ticket.showtimeLanguage === "DUB"
                      ? "Lồng tiếng"
                      : ticket.showtimeLanguage === "ENGLISH"
                      ? "Tiếng Anh"
                      : "VietSub"}
                  </span>
                )}
          ...
          <div className="px-4 mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {ticket.seats.map((seat) => {
                const seatTypeKey = SEAT_TYPE_LABEL[seat.type]
                  ? seat.type
                  : "standard";
                return (
                  <div
                  ...
                  >
                    <span
                      style={{
                        fontSize: 14,
                        fontWeight: 800,
                        color: SEAT_TYPE_COLOR[seatTypeKey],
                      }}
                    >
                      {seat.id}
                    </span>
        ...
        <div
          ...
          style={{ background: "transparent", height: 24 }}
        >
          {/* Left notch */}
          <div
          ...
          {/* Dashed line */}
          <div
            className="flex-1 mx-4 border-dashed"
        ...
        {/* Bottom section */}
        <div
          className="rounded-b-3xl px-5 pt-4 pb-5"
          style={{ background: "#111122" }}
        >
          ...
          <div className="flex flex-col items-center">
            ...
            {isPaid ? (
              ...
              >
                Thanh toán để mở mã QR check-in
              </div>
            )}
          </div>
      ...
      <p
        ...
        style={{ fontSize: 12, color: "rgba(255,255,255,0.2)", maxWidth: 300 }}
      >
        Vé chỉ sử dụng một lần. Không được chuyển nhượng hoặc hoàn trả sau khi
        đã thanh toán.
      </p>
    </div>
  );
